...
type
  ...
  TQueryDocFind = function (Sender: TObject; Date: TDateTime; Values: TStrings): Cardinal of object;
...
if FChanged then DoChanged;
end;

procedure TGridFields.DoChanged;
begin
FChanged := true;
if not FDisabledControls then begin
  if Assigned(FOnChanged) then FOnChanged(Self);
  FChanged := false;
end;
end;
...
for Index := 0 to Count-1 do begin
  LItem := Items[Index];
  if not LItem.Equals(Sender) then begin
    if LItem.Column=NewColumn then begin
...
var LItem: TGridField;
begin
LItem := Find(Name);
if Assigned(LItem) then Result := LItem.Column
else Result := -1;
end;
...
begin
if Count>0 then Result := Add(Name, Title, Items[Count-1].Column+1)
else Result := Add(Name, Title, 0);
end;
...
const ccolor: array[boolean]of TColor = (clWindowText, clGreen);
var Index: integer;
  Field: TField;
begin
...
if Assigned(FOnChangeReadOnly) then FOnChangeReadOnly(Self);
end;
...
for Index := 0 to FFields.Count-1 do begin
  ...
  if ColCount<=LField.Column then ColCount := LField.Column+1;
  Cells[LField.Column,0] := LField.Title;
  if LField.Width>0 then ColWidths[LField.Column] := LField.Width;
end;
...
if RowCount=2 then begin
  if GetExists(1) then Result := 1;
end
else if RowCount>2 then begin
  Result := RowCount-1;
  if not GetExists(Result) then Dec(Result);
end;
end;

function TStringGrid.GetValue(Index: integer; ValueName: string): string;
var Field: TGridField;
begin
Result := '';
Field := FFields.Find(ValueName);
if Assigned(Field) then
if (Field.Column>=0) and (Field.Column<ColCount) then
if (Index>=0) and (Index<(RowCount-1)) then begin
...
var n: integer;
begin
if (Index>=0) and (Index<RowCount) then begin
  Inc(Index);
  if RowCount<(Index+1) then RowCount := Index+1;
  for n := 0 to FFields.Count-1 do begin
...
begin
case Key of
  40: if (Shift=[]) and (Row=(RowCount-1)) then begin //Down
    if GetExists(RowCount-1) then begin
      RowCount := RowCount+1;
  ...
  38: if (Shift=[]) and ((RowCount>2) and (Row=(RowCount-1))) then begin  //Up
    if not GetExists(RowCount-1) then begin
      RowCount := RowCount-1;
      Row := RowCount-1;
      exit;
    end;
  end;
  46: if (Shift=[ssCtrl]) and (Row>0) then begin //Del
    DoDeleteRow(Row);
  end;
  45: if (Shift=[ssCtrl]) and (Row>0) then begin //Insert
    DoInsertRow(Row);
  end;
end;
...
begin
Result := false;
if pos(#13#10, Trim(Rows[RowCount-1].Text))>0 then begin
  Result := true;
end;
end;
...
var n: integer;
begin
if (ARow=1) and (RowCount=2) then begin
  Rows[1].Clear;
end
else if (ARow>0) and (RowCount>2) then begin
  if (ARow<(RowCount-1)) then
  for n := ARow to RowCount-2 do begin
...
begin
if (ARow>0) then
if GetExists(ARow) then begin
  RowCount := RowCount+1;
  for n := RowCount-1 downto ARow+1 do begin
    Rows[n].Assign(Rows[n-1]);
  end;
  Rows[ARow].Clear;
end;
...
while source<>'' do begin
  if getvalue(source, buff) then begin
    n := Length(FHeader);
...
begin
lcol := 0;
if grid.RowCount<=row then grid.RowCount := row+1;
grid.Rows[row].Clear;
while (lcol<Length(FHeader)) and (source<>'') do begin
  if getvalue(source, buff) then begin
    with FHeader[lcol] do
    if Column>=0 then grid.Cells[Column, row] := buff;
  end;
  Inc(lcol);
end;
end;
...
try
  ...
  WriteLn(f, '');
  for lrow := 1 to Source.RowCount-1 do begin
    for n := 0 to Fields.Count-1 do begin
      fld := Fields[n];

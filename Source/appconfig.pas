...
for j := 1 to ParamCount do begin
  ...
  if lparam<>'' then begin
    if CharInSet(lparam[1], ['-', '\', '/']) then lparam := copy(lparam,2);
    for n := 0 to Length(ParamNames)-1 do
    if lparam=AnsiLowerCase(ParamNames[n]) then begin
      Result := true;
      exit;
    end;
  end;
...
begin
if AnsiLowerCase(FFilename)<>AnsiLowerCase(AFilename) then begin
  Close;
  FFilename := AFilename;
end;
end;
...
else if FAutoOpen then if Open then begin
  Result := (Section<>'') and (Ident<>'');
end;
end;
...
begin
if Check(Section,Ident) then begin
...
if Check(Section,Ident) then begin
  ...
  if expand4int(buff, @fpos[0], 4)>3  then begin
    Def.Left := fpos[0];
    Def.Top := fpos[1];
    Def.Width := fpos[2];
    Def.Height := fpos[3];
...
if Check(Section,Ident) then begin
  Def.DisableControls;
  for j := 0 to Def.Count-1 do begin
    fld := Def[j];
    buff := DoReadString(Section+'\'+Ident, fld.Name, '');
    fpos[0] := fld.Column;
    fpos[1] := fld.Width;
    if expand4int(buff, @fpos[0], 2)>1 then begin
      fld.Column := fpos[0];
      fld.Width := fpos[1];
    end;
  end;
...
if Check(Section,Ident) then begin
  for j := 0 to Def.Columns.Count-1 do begin
    col := Def.Columns[j];
    w := Read(Section+'\'+Ident, col.FieldName, col.Width);
    if w>(Def.Width-40) then w := (Def.Width-40);
    col.Width := w;
  end;
end;
end;
...
begin
if Check(Section,Ident) then begin
  if Value.WindowState=wsNormal then begin
    ...
    if expand4int(buff, @fpos[0], 5)>3 then begin
...
if Check(Section,Ident) then begin
  Value.Realign;
  for j := 0 to Value.Count-1 do begin
    fld := Value[j];
...
if Check(Section,Ident) then begin
  for j := 0 to Value.Columns.Count-1 do begin
    col := Value.Columns[j];

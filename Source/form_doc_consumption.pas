...
uses Classes, Windows, SysUtils, Vcl.StdCtrls, Grids.Helper, form_doccreate, form_dm, strtools, form_findleftovers, appconfig, docparam;
...
begin
  Result := 'параметры (склад/место хранения/партия) применяются'#13#10 + 'к строкам таблицы если в таблице параметр не задан'#13#10 +
    #13#10 + 'остатки сортируются по дате'#13#10 + 'если партия задана, то товар выбирается из этой поставки'#13#10 + #13#10 +
    inherited edithelp_info;
end;

function Tfrm_doc_consumption.DoQueryDoc(Sender: TGoods; Param: integer): Cardinal;
begin
  Result := inherited DoQueryDoc(Sender, get_dict_selected);
end;
...
function Tfrm_doc_consumption.DoExecuteDoc(DocGoods: Cardinal; DocGoodsCount: integer;
  DocShipment, DocWarehouse, DocPlace: Cardinal): integer;
var
  DocId: Cardinal;
begin
  Result := 0;
  DocGoodsCount := abs(DocGoodsCount);
  if DocGoodsCount > 0 then
    begin
      DocId := FDM.insert_doc(DocType, DocName, DocDate, DocShipment, DocWarehouse, DocPlace);
      if DocId > 0 then
        begin
          Result := 1;
          if FDM.insert_doclink(DocId, DocGoods, -DocGoodsCount) > 0 then
            begin
              Inc(Result);
            end;
        end;
...
begin
  ...
  if Index < 0 then
    begin
      with Goods.Header.Warehouse do
        Id := FDM.find_value2('Warehouse', Id.ToString, Name);
      with Goods.Header.Place do
        Id := FDM.find_value2('Place', Id.ToString, Name);
      with Goods.Header.Shipment do
        Id := FDM.find_value2('Shipment', Id.ToString, Name);
    end
  else begin
      with Goods.Dictionary.Warehouse do
        Id := FDM.find_value2('Warehouse', '', Name);
      with Goods.Dictionary.Place do
        Id := FDM.find_value2('Place', '', Name);
      with Goods.Dictionary.Shipment do
        Id := FDM.find_value2('Shipment', '', Name);
    end;
end;
...
begin
  if (not FDM.Connected) or (DocCount < 1) then
    exit;
  Goods := TGoods.Create;
  try
    with FDM do
          ...
          begin
            ...
            if Goods.Count > 0 then
              begin
                ...
                while (Goods.Id > 0) and (Goods.Count > 0) do
                  begin
                    if FindCount > Goods.Count then
                      FindCount := Goods.Count;
            ...
            if Goods.Count > 0 then
              begin
                MessageBox(Format('Невозможно расходовать %d единиц'#13#10'наименование "%s"'#13#10'артикул "%s"',
                  [Goods.Count, Goods.Name, Goods.Article]), MB_ICONINFORMATION + MB_OK);
              end;
          end;
        ProgressEvent(1);
        if not conn then
...
var
  Goods: TGoods;
begin
  with FDM.FDQuery_findleftovers do
    begin
      if Active then
        Close;
      Goods := TGoods.Create;
      try
        ...
        with TDbForm_findleftovers(OpenChildForm(TDbForm_findleftovers)) do
          begin
            Config := Self.Config;

...
begin
  ...
  with TFormHelper.CreateLabel(obj, alLeft, lbl_place.Width, 30, 'Ќовый склад/место хранени€') do
    begin
      WordWrap := true;
      Font.Size := 8;
    end;
...
begin
  Result := 'формируетс€ два одинаковых документа'#13#10 + 'с отрицательным количеством дл€ текужего склада'#13#10 +
    'с положительным количеством дл€ нового склада'#13#10 + #13#10 + inherited edithelp_info;
end;

procedure Tfrm_doc_moving.act_doc_createExecute(Sender: TObject);
begin
  if (get_key_dict(fld_newwarehouse) > 0) and (get_key_dict(fld_newplace) > 0) then
    begin
...
begin
  ...
  if DocGoodsCount > 0 then
    begin
      ...
      if (newDocPlace > 0) or (newDocPlace > 0) then
        begin

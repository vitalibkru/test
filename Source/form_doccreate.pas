...
uses Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Menus, Vcl.Graphics, Vcl.Controls, Vcl.Forms,
  Vcl.Dialogs, Vcl.Buttons, Vcl.Grids, Vcl.StdCtrls, Vcl.ComCtrls, Vcl.ExtCtrls, System.ImageList, Vcl.ImgList, System.Actions,
...
type
  ...
  protected
    ...
    function DoQueryDoc(Sender: TGoods; Param: integer): Cardinal; virtual;
    function get_key_dict(List: TComboBox): Cardinal;
    function get_dict_selected: Cardinal;
...
begin
  ...
  fld_grid.Fields.Add(cname_goods_id, '№').Width := 60;
  fld_grid.Fields.Add(cname_shipment_name, 'Партия товара').Width := 160;
  fld_grid.Fields.Add(cname_warehouse_name, 'Склад').Width := 160;
  fld_grid.Fields.Add(cname_place_name, 'Место хранения').Width := 160;
  fld_grid.Fields.Add(cname_goods_name, 'Наименование').Width := 160;
  fld_grid.Fields.Add(cname_goods_article, 'Артикул').Width := 160;
  fld_grid.Fields.Add(cname_goods_count, 'Количество').Width := 80;
  fld_grid.Fields.Add(cname_goods_size, 'Размер').Width := 80;
  fld_grid.Fields.Add(cname_goods_cost, 'Цена').Width := 60;
...
begin
  if Index < 0 then
    begin
      ...
      Goods.Clear;
      Goods.Dictionary.Warehouse.Name := fld_grid.GetValue(Index, cname_warehouse_name);
      Goods.Dictionary.Place.Name := fld_grid.GetValue(Index, cname_place_name);
      Goods.Dictionary.Shipment.Name := fld_grid.GetValue(Index, cname_shipment_name);
      Goods.Id := str2uint(fld_grid.GetValue(Index, cname_goods_id));
      Goods.Name := fld_grid.GetValue(Index, cname_goods_name);
      Goods.Article := fld_grid.GetValue(Index, cname_goods_article);
      Goods.Count := str2int(fld_grid.GetValue(Index, cname_goods_count));
      Goods.Size := str2float(fld_grid.GetValue(Index, cname_goods_size));
      Goods.Cost := str2float(fld_grid.GetValue(Index, cname_goods_cost));
    end;
end;
...
begin
  Result := 'Редактирование списка:'#13#10 + '<Down> - на последней строке добавится новая'#13#10 + '<Ctrl> + <Del> - удалить строку'#13#10
...
function Tfrm_doccreate.DoQueryDoc(Sender: TGoods; Param: integer): Cardinal;
begin
  Result := 0;
  with Sender.Header.Shipment do
    begin
      Id := get_key_dict(fld_shipment);
      Name := Trim(fld_shipment.Text);
    end;
  with Sender.Header.Place do
    begin
      Id := get_key_dict(fld_place);
      Name := Trim(fld_place.Text);
    end;
  with Sender.Header.Warehouse do
    begin
      Id := get_key_dict(fld_warehouse);
      Name := Trim(fld_warehouse.Text);
    end;
  if Assigned(FOnQueryDoc) then
    Result := FOnQueryDoc(Sender, fld_docdate.Date, Param);
end;
...
begin
  Goods := TGoods.Create;
  try
    if DoQueryDoc(Goods, 0) > 0 then
      begin
        Values := TStringList.Create;
        try
          Goods.AssignTo(Values);
          fld_grid.SetValues(fld_grid.Count, Values);
        finally
          Values.Free;
        end;
      end;
...
begin
  if Assigned(FQueryDictionary) then
    FQueryDictionary(DictName, DictData, Where);
end;

function Tfrm_doccreate.get_dict_selected: Cardinal;
begin
  Result := $00;
  if get_key_dict(fld_shipment) > 0 then
    Result := Result or $01;
  if get_key_dict(fld_warehouse) > 0 then
    Result := Result or $02;
  if get_key_dict(fld_place) > 0 then
    Result := Result or $04;
...
begin
  ...
  if (LIndex < 1) and (Sender.Items.Count > 1) then
    LIndex := 1;
  if LIndex >= Sender.Items.Count then
    LIndex := Sender.Items.Count - 1;

...
uses Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs,
  Vcl.ComCtrls, Vcl.StdCtrls, Vcl.ExtCtrls, System.Actions, Vcl.ActnList, System.ImageList, Vcl.ImgList, Vcl.Buttons, Data.DB, Vcl.Grids,
  Vcl.DBGrids, FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf,
  FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet, FireDAC.Comp.Client, FireDAC.UI.Intf, FireDAC.Stan.Def,
  FireDAC.Stan.Pool, FireDAC.Phys, FireDAC.Phys.MSSQL, FireDAC.Phys.MSSQLDef, FireDAC.VCLUI.Wait, appconfig;
...
begin
  if chk_file_refresh.Checked then
...
begin
  if FDQuery_Goods.Active then
    FDQuery_Goods.Close;
  if FDQuery_GoodsDoc.Active then
    FDQuery_GoodsDoc.Close;
...
begin
  if Assigned(FDataSource.DataSet) then
    begin
      if FDataSource.DataSet.Equals(FDQuery_GoodsDoc) then
        ds_goodsdocRefresh()
      else
        if FDataSource.DataSet.Equals(FDQuery_Goods) then
          ds_goodsRefresh();
    end;
end;
...
begin
  ...
  case Index of
    0:
      Result := Trim(fld_warehouse.Text); // WarehouseId
    1:
      Result := Trim(fld_place.Text); // PlaceId
    2:
      Result := Trim(fld_shipment.Text); // ShipmentId
  end;
end;
...
begin
  ...
  case Index of
    0:
      Result := get_key_dict(fld_warehouse); // WarehouseId
    1:
      Result := get_key_dict(fld_place); // PlaceId
    2:
      Result := get_key_dict(fld_shipment); // ShipmentId
  end;
end;
...
begin
  case Index of
    0:
      set_key_dict(fld_warehouse, KeyDict); // WarehouseId
    1:
      set_key_dict(fld_place, KeyDict); // PlaceId
    2:
      set_key_dict(fld_shipment, KeyDict); // ShipmentId
  end;
end;

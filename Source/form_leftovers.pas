...
uses Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs,
  Vcl.Grids, Vcl.ComCtrls, Vcl.StdCtrls, Vcl.ExtCtrls, Data.DB, Vcl.DBCtrls, Vcl.DBGrids, FireDAC.Stan.Intf, FireDAC.Stan.Option,
  ...
  System.ImageList, Vcl.ImgList, System.Actions, Vcl.ActnList, FireDAC.Comp.Client, FireDAC.Comp.DataSet, Vcl.Buttons, Grids.Helper,
  appconfig;
...
begin
  if FDQuery_Leftovers.Active then
    FDQuery_Leftovers.Close;
...
const
  bint: array [boolean] of integer = (1, 0);
begin
  FChanged := true;
  if FDisabledControls then
    exit;
  DisableControls;
  if Connected then
    begin
...
begin
  Result := 0;
  if act_group_warehouse.Checked then
    Result := Result or $01;
  if act_group_place.Checked then
    Result := Result or $02;
  if act_group_shipment.Checked then
    Result := Result or $04;
  if act_group_goods.Checked then
    Result := Result or $08;
end;
...
begin
  if FQueryDateTime <> DocDate then
    begin
      act_file_refresh.Execute;
    end;
end;
...
begin
  ...
  if fld_doc_time.Checked then
    Result := Result + fld_doc_time.Time;
end;
